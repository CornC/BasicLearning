'''
@Description: 由于直接获取百度学术数据的方法有些问题，现试试用 selenium 进行模拟浏览器拉取
@version: 
@Company: Lifegen
@Author: CornC.fcx
@Date: 2019-05-31 13:45:08
@LastEditors: CornC.fcx
@LastEditTime: 2019-05-31 14:14:49
'''
...
def get_all_data(urls_list):
    ...
    for url in urls_list:
        title, authors, abstract, publish_text, year, publish, ref_wr = get_item_info(url)
...
def save_csv(dit):
    ...
    columns = ["title", "authors", "abstract", "publish_text", "year", "publish", "ref_wr"]
    data.to_csv("abstract_data.csv", index=False, columns=columns)
    print("That's OK!")

if __name__ == "__main__":
    key_word = "牛肉品质"
    # proxy = '202.112.237.102:3128'
    soup = driver_open(key_word)
    urls_list = page_url_list(soup, page=2)
